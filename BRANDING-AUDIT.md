...
5. **`CNAME` file created** (`baxter.solutions`) — didn't exist before. GitHub Pages' custom-domain setting was previously configured via the API only, with no repo file backing it, which risks the association being dropped on a future force-push deploy. This is repo content, not a DNS change.
